    ...
    extends Agent
{
	...
	public Node minimaxSearch(Node node,
                              int depth)
	{
		Node bestChild = node;
		bestChild = maxValueState(bestChild, depth);
		return bestChild;
	}

	private Node maxValueState(Node node, int depth){
		Node bestChild = node;
		if (bestChild.isTerminalState() || depth == this.getMaxDepth())
		{
			bestChild.setFootmanUtilityValue(Heuristic.getFootmanHeuristicValue(bestChild));
			return bestChild;
		} 
		Node empty = null;
		List<Node> allChildren = bestChild.getChildren();
		double bestValue = Double.NEGATIVE_INFINITY;
		for (Node child: allChildren){
			Node answer = minValueState(child, depth + 1);
			double value = answer.getFootmanUtilityValue();
			if (value > bestValue){
				bestValue = value;
				empty = answer;
			}
	...
	private Node minValueState(Node node, int depth){
		Node bestChild = node;
		if (bestChild.isTerminalState() || depth == this.getMaxDepth())
		{
			bestChild.setFootmanUtilityValue(Heuristic.getFootmanHeuristicValue(bestChild));
		...
		for (Node child: allChildren){
			Node answer = maxValueState(child, depth + 1);
			double value = answer.getFootmanUtilityValue();
			if (value < bestValue){
				bestValue = value;
				empty = answer;
