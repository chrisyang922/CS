	...
	extends Agent
{
                                  ...
                                  StateView state)
    {
        ...
        int randomNumber = index.nextInt(validNeighbors.size());
        Vertex theVertex = validNeighbors.get(randomNumber);
        if (DistanceMetric.chebyshevDistance(goal, src) >= DistanceMetric.chebyshevDistance(goal, theVertex)){
            Random prob = new Random();
            double randomDouble = prob.nextDouble();
            if (getTemperature(getTime()) > randomDouble){
                return theVertex;
            }
            else{
                return getNextPosition(src, goal, state);
            }
        }else{
            return theVertex;
        }
