...
public class DisrespectfulStealthAgent
    extends Agent
{
    ...
    private int goldXLocation;
    private int goldYLocation;
    private boolean isGoldTaken;
    private boolean goldTakenTripFirstTime;
    private int count;
    ...
    public DisrespectfulStealthAgent(int playerNum, String[] args)
    {
        ...
        this.goldXLocation = 0;
        this.goldYLocation = 0;
        this.isGoldTaken = false;
        this.goldTakenTripFirstTime = false;
        this.count = 0;
    }
    ...
    public int getGoldXLocation() {return this.goldXLocation;}
    public int getGoldYLocation() {return this.goldYLocation;}
    public boolean getIsGoldTaken() {return this.isGoldTaken;}
    public boolean getGoldTakenTripFirstTime() {return this.goldTakenTripFirstTime;}
    ...
    private void setGoldXLocation(int i) {this.goldXLocation = i;}
    private void setGoldYLocation(int i) {this.goldYLocation = i;}
    private void setGoldTaken(boolean b) {this.isGoldTaken = b;}
    private void setGoldTakenTripFirstTime(boolean b) {this.goldTakenTripFirstTime = b;}
    private void addCount(){this.count += 1;}
    private void nullCount(){this.count = 0;}
                                            ...
                                            HistoryView history)
    {
        ...
        this.setGoldTakenTripFirstTime(true);
                                           ...
                                           HistoryView history)
    {
        ...
        if (this.getDestroyedBase() == true)
        {
            ...
            if(this.getCameBackToBase() == true)
            {
        ...
        if (state.getResourceNode(580) != null)
        {
            ResourceView gold = state.getResourceNode(580);
                
            this.setGoldXLocation(gold.getXPosition());
            this.setGoldYLocation(gold.getYPosition());
            Vertex goldDestination = new Vertex(this.getGoldXLocation(), this.getGoldYLocation());
            System.out.println("Gold location is " + this.getGoldXLocation() + " " + this.getGoldYLocation());
            UnitView player = state.getUnit(this.getMyUnitID());
            int xPosPlayer = player.getXPosition();
            int yPosPlyaer = player.getYPosition();
            Vertex playerVertex = new Vertex(xPosPlayer, yPosPlyaer);
            Set<Vertex> goldNeighbors = this.getOutgoingNeighbors(goldDestination, state);
            boolean hurray = false;
            


            for (Vertex goldNeighbor: goldNeighbors)
            {
                System.out.println(goldNeighbor.getXCoordinate() + " " + goldNeighbor.getYCoordinate());
                if (playerVertex.equals(goldNeighbor)){
                    hurray = true;
                    if (state.isResourceAt(this.getGoldXLocation(), this.getGoldYLocation()) == false)
                    {
                        this.setGoldTaken(true);
                    }
                    
                    break;
                }
            }
            if (this.getIsGoldTaken() || hurray == true)
            {
                
                Action action = Action.createPrimitiveGather(this.getMyUnitID(), this.getDirection(playerVertex, goldDestination));
                System.out.println(this.getDirection(playerVertex, goldDestination));

                actions.put(this.getMyUnitID(), action);
                System.out.println("Took the gold");

                
            }
            else
            {
                if (this.getGoldTakenTripFirstTime() == true)
                {
                    this.setGoldTakenTripFirstTime(false);
                    this.setInitialPosX(xPosPlayer);
                    this.setInitialPosY(yPosPlyaer);
                    StateView currentState = state;
                    Path path = this.search(playerVertex, goldDestination, currentState);
                    Stack<Path> stack = new Stack<>();
                    Path finalPath = path;
                    Path copyPath = path;
                    Stack<Vertex> inOrderVertex = new Stack<Vertex>();
                    Stack<Direction> inOrderDirection = new Stack<Direction>();
                    System.out.println("basic gold");
                    if (path == null)
                    {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        Path path = search(playerVertex, goldDestination, currentState);
                        Stack<Path> stack = new Stack<>();
                        Path finalPath = path;
                        Path copyPath = path;
                        Stack<Vertex> inOrderVertex = new Stack<Vertex>();
        ...
        else
        {
            if (this.getFirstTime() == true){
                ...
                StateView currentState = state;
                System.out.println(playerVertex.getXCoordinate() + " " + playerVertex.getYCoordinate());
                Path path = search(playerVertex, townHallVertex, currentState);
                System.out.println(townHallVertex.getXCoordinate() + " " + townHallVertex.getYCoordinate());
                Stack<Path> stack = new Stack<>();
                Path finalPath = path;
                Path copyPath = path;
                Stack<Vertex> inOrderVertex = new Stack<Vertex>();
                                            ...
                                            StateView state)
    {
        ...
        UnitView townHall = state.getUnit(this.getEnemyTownhallUnitID());
        int maxIsTheGold = 0;
            for(int x: state.getAllResourceIds())
            {
                if (x > maxIsTheGold)
                {
                    maxIsTheGold = x;
        ...
        int goldX = this.getGoldXLocation();
        int goldY = this.getGoldYLocation(); 
        if (townHall != null)
        {
            townHallX = townHall.getXPosition();
            townHallY = townHall.getYPosition();
        }
        ...
        for (Vertex neighbor : neighborsneighbor)
        {
            ...
            if(state.inBounds(newXCoor, newYCoor) && (!state.isResourceAt(newXCoor, newYCoor) || (newXCoor == goldX && newYCoor == goldY)))
            {
                       ...
                       StateView state)
    {
        ...
        while(!vertexQueue.isEmpty())
        {
            ...
            if (currentPP.getDestination().equals(goalVertex))
            {
                ...
                //System.out.println("my current location is " + currentPP.getDestination().getXCoordinate() + " " + currentPP.getDestination().getYCoordinate());
            ...
            for (Vertex v: neighborsOfCurrentPP)
            {
                if (!visited.contains(v))
                {
                    ...
                    //System.out.println("my current location is " + currentPP.getDestination().getXCoordinate() + " " + currentPP.getDestination().getYCoordinate());
                    //System.out.println("The destination is " + v.getXCoordinate() + " " + v.getYCoordinate() + " and the edge cost is " + newScore);                    
